...
def scrape_movies(url, page):
    ...
    for movie in movies:
        try:
            ...
            # Extract genres
            genres = ', '.join([genre.get_text(strip=True) for genre in movie.find('p', class_='genre').find_all('a', class_='filterLink')])
            if not genres:  # If genres are not found, print a message
                print(f"No genres found for {title}.")
            else:
                print(f"Genres for {title}: {genres}")  # Debugging line
            ...
            if not streaming_availability:  # If no streaming services are found
                print(f"No streaming services found for {title}.")
            else:
                print(f"Streaming services for {title}: {streaming_availability}")  # Debugging line
